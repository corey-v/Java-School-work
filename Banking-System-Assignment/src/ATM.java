...
public class ATM {
	...
	public void initialize() {
		bank.createAccount(new BankCustomer("Jerry", "Seinfeld", "213", "h2as2"));
		bank.createAccount(new BankCustomer("Harry", "Jones", "324", "ju23a"));
		bank.createAccount(new BankCustomer("Jason", "Kruger", "543", "ki39a"));
	}
	...
	public void transactDeposit() {
		if(isSignedIn) {
			...
			bank.deposit(theCustomer.getAccountNumber(), amount);
			printCommands();
		}else {
	...
	public void transactWithdraw() {
		if(isSignedIn) {
			...
			double amount = parser.getDoubleInput();
			bank.withdraw(theCustomer.getAccountNumber(), amount);
			printCommands();
		}else {
			System.out.println("ERROR: You must LOGIN before you can perform a transaction");
			verifyCustomer();
