...
import com.a00918598.data.Account;
import com.a00918598.data.Bank;
import com.a00918598.data.BankCustomer;
import com.a00918598.data.ChequingAccount;
import com.a00918598.data.GoldAccount;
...
public class ATM {
	...
	private BankReport bankReportInterface;
	...
	public ATM() {
		...
		this.bankReportInterface = new BankReport();
		initialize();
	}
        ...
        while (! finished) {
        	...
        	switch(command) {
            	...
            	case 5: finished = true;
            			break;
            	case 6: printDisplayByCode();
            			break;
            	case 7: bankReportInterface.displayAllCodes(Bank.theBank);
            			printBankCommands();
            			break;
            	case 8: bankReportInterface.displayInactiveCodes(Bank.theBank);
            			printBankCommands();
            			break;
            	case 9: bankReportInterface.displayAccountTotals(Bank.theBank);
            			printBankCommands();
            			break;
        	}
	...
	public void verifyCustomer() {
		...
		if(isValidString(accountNumber) && isValidString(passCode) && accountNumber.equals("admin") && passCode.equals("admin")) {
			printBankCommands();
		}else if(isValidString(accountNumber) && isValidString(passCode) && Bank.theBank.containsKey(accountNumber)
				&& Bank.theBank.get(accountNumber).getPassCode().equals(passCode) 
				&& Bank.theBank.get(accountNumber).getMyAccount().getActive()) {
	...
	/*
	 * Prints the bank employee's menu
	 */
	private void printBankCommands() {
		System.out.println("Choose one of the following options:");
		System.out.println("6 - Display Customers By Account Type");
		System.out.println("7 - Display All Active Accounts");
		System.out.println("8 - Display Inactive Accounts");
		System.out.println("9 - Display Account Totals");
		System.out.println("5 - Exit");
		System.out.print(">");
	}
	
	/**
	 * Prints the request for the account prefix
	 */
	public void printDisplayByCode() {
		System.out.println("Enter the account type to display:");
		String actType = parser.getStringInput();
		bankReportInterface.displayByCode(Bank.theBank, actType);
		//bank.withdraw(theCustomer.getMyAccount().getAccountNumber(), amount);
		printBankCommands();
	}
